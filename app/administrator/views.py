...
from tools.decorators import admin_required,permission_required
...
# 选课前的准备
@administrator.route('/pre_book_lesson',methods=['GET','POST'])
@login_required
@permission_required(Permission.BOOK_LESSON_FOR_OTHERS)
def pre_book_lesson():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('administrator.book_lesson',student_username=student_username,teacher_username=teacher_username))
    return render_template('administrator/pre_book_lesson.html',form=form)

# 选课视图
@administrator.route('/book_lesson/<student_username>/<teacher_username>',methods=['GET','POST'])
@login_required
@permission_required(Permission.BOOK_LESSON_FOR_OTHERS)
def book_lesson(student_username,teacher_username):
    ...
    if teacher and (student.role_id==1 or student.role_id==2):
        ...
        return render_template('administrator/book_lesson.html',
        student=student,
        teacher=teacher,
        this_week=this_week,
...
# 取消已选的课程
@administrator.route('/cancel',methods=['GET','POST'])
@login_required
@permission_required(Permission.BOOK_LESSON_FOR_OTHERS)
def cancel():
...
# 修改游客、学生和老师的个人信息的预处理视图
@administrator.route('/pre_modify_info')
@login_required
...
def pre_modify_info():
    ...
    return render_template('administrator/pre_modify_info.html',all_visitors=all_visitors,all_students=all_students,all_teachers=all_teachers)


# 修改游客、学生和老师的个人信息
@administrator.route('/modify_personal_info/<username>',methods=['GET','POST'])
@login_required
@permission_required(Permission.MODIFY_INFO_FOR_OTHERS)
def modify_personal_info(username):
    ...
    if user and user.role_id in {1,2,3}:
        ...
        return render_template('administrator/modify_personal_info.html',form=form)
    flash('用户不存在')
    return redirect(url_for('main.personal_center'))
